...
public class UserAuthController {
    ...
    @PostMapping("/sign-up")
    public ResponseEntity<UserAuthResponse> performSignUp(@RequestBody UserSignUpRequest req) {
        UserAuthResponse response = authServices.performSignUp(req);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/web-admin-login")
    public ResponseEntity<UserAuthResponse> performWebAdminLogin(@RequestBody UserAuthRequest req) {
        log.info("performWebAdminLogin :: {} ", req);
        UserAuthResponse response = authServices.performWebAdminLogin(req);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/customer-login")
    public ResponseEntity<UserAuthResponse> performCustomerLogin(@RequestBody UserAuthRequest req) {
        UserAuthResponse response = authServices.performCustomerLogin(req);
        return ResponseEntity.ok(response);
    }
