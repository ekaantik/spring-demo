...
//                    .setIssuedAt(today) // for example, now
        ...
        claims.put("custId", user.getCustomerDetails().getId());

        // Build user roles with necessary prefix and add to token claims
        List<String> roleList = new ArrayList<>();
//        for (Role role : user.getRoles()) {
//            roleList.add("ROLE_" + role.getName());
//        }


        for (UserRoles  role : user.getUserRolesList()) {
            roleList.add("ROLE_" + role.getRole().getName());
        }
//        claims.put("scope",roleList.toString());
        claims.put("roles", roleList.toString());

        // Generate JWT token with provided claims and signing key
        String token = Jwts.builder()
                .setIssuer("InfraRedPower")
//                .setSubject("token")
//                .setAudience("you")  // set customerId as audiance
                .setClaims(claims)
